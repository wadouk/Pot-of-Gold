...
const CLASS_NAME = "AutoComplete";
const CONTRACT_ID = "@mozilla.org/autocomplete/search;1?name=autocomplete";
...
SimpleAutoCompleteSearch.prototype = {
  ...
  startSearch: function(searchString, searchParam, result, listener) {
    ...
    if (searchParam.length > 0) {
      ...
      var params = nativeJSON.decode(searchParam);
      var fullSearchString = searchString;
      searchString = fullSearchString.substring(fullSearchString.lastIndexOf(",")+1);
      //var searchResults = nativeJSON.decode(searchParam);
      var searchResults = myDb.mDBConn.createStatement(params.query);
      //log(searchResults);
      //dump2(searchResults.value.length);
      var results = [];
      var comments = [];
      while (searchResults.executeStep()) {
      	      if (searchResults.row.value.indexOf(searchString) == 0) {
      	      	      results.push(fullSearchString + "," + searchResults.row.value);
      	      	      comments.push(searchResults.row.value);
      	      }
